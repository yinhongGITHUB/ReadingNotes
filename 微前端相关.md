...
4. 主应用页面预留子应用挂载点,例如：SubAppContainer.vue 组件：

```js
<div id="subapp-viewport"></div>
```

5. 主应用的路由文件需要配置子应用挂载点（主应用路由只管渲染哪个容器，子应用内部路由由子应用自己管理）：
...
const routes = [
  {
    path: "/app1/:catchAll(.*)*", // 匹配所有 /app1 开头的路由，交给子应用内部处理
    component: () => import("@/views/SubAppContainer.vue"), // 只负责渲染挂载容器
  },
  {
    path: "/app2/:catchAll(.*)*",
...
> `SubAppContainer.vue` 里放 `<div id="subapp-viewport"></div>` 作为挂载容器，qiankun 会把子应用渲染进去。注意：**不是直接引入子应用的 index.html**。

##### 第二步：改造子应用的 main.ts（独立运行 / 被主应用加载）

1. 在每个子应用的 main.ts 里适配 qiankun：
   **这里直接 props.container.querySelector("#app") 是因为 qiankun 加载子应用后，会把子应用的页面内容（通常是 <div id="app"></div> 及其内容）插入到 #subapp-viewport 这个容器里。**
...
// 挂载 ---> 触发时机：当主应用路由切换到子应用对应的 activeRule 时，qiankun 会调用子应用的 mount 方法
export async function mount(props) {
  render(props);
}

// 卸载 ---> 触发时机：同一挂载容器里切换到另一个子应用（前一个会被卸载）
...
#### 二、qiankun 的环境变量 window.`__POWERED_BY_QIANKUN__`
