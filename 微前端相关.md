...
- qiankun 默认提供两种样式隔离能力：
  - strictStyleIsolation: true 启用 Shadow DOM 隔离（前提是子应用根节点支持 attachShadow）。
...
- qiankun 默认开启 JS 沙箱，子应用运行时会劫持 window 对象，防止全局变量污染。
- 通过 sandbox 配置可选择快照沙箱、Proxy 沙箱等不同隔离级别。

2. **props 通信**

- 主应用通过 props 向子应用传递数据，子应用通过生命周期 mount(props) 获取。
- 数据单向流动，互不影响。
