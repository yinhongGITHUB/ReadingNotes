...
- **ABAC（基于属性）**：用用户/资源/环境属性 + 策略规则动态决策，灵活细粒度，需策略引擎与审计。
  ...
  - 通俗规则例子：
    - 规则1：同部门的人才能看本部门文件（user.department == resource.department 且 action==read）。
    - 规则2：写入操作仅限工作时间且必须内网（action==write 且 time 在 09:00-18:00 且 IP 属于公司网段）。
    - 规则3：机密文件只允许“正式员工+已开双因子”访问，实习生或未开 2FA 一律拒绝。
