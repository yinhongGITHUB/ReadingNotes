...
1. 如何触发 Service-Worker 更新
   ...
   （只能手动更改，然后重新部署 sw.js 文件）并且 后续的 caches.open 的时候 都用这个新的版本号
...
**哈希表的哈希冲突问题：** 链地址法（其实就是哈希函数对应一个二维数组，二维数组就可以同一个 index 存储多个不同给的数据）
...
**可打印字符：**能够在屏幕、纸张等媒介上显示出来的字符。它们有具体的图形表现，可以被人看到和识别。

常见范围：

英文：字母（A-Z, a-z）、数字（0-9）、标点符号（如 !@#$%^&\*() 等）
中文：汉字、标点等
空格也算可打印字符（虽然看不见，但有宽度）

**不可打印字符：**如换行符（\n）、回车符（\r）、制表符（\t）、控制字符等，这些不会直接显示为图形。
...
1. 语法：用 import 和 export（export default），更简洁直观。
2. 加载是**异步的**，适合浏览器和服务端。
3. 导出是“实时绑定”，导入的变量会随原模块变化而变化（简单数据类型还是引用数据类型）。
...
- `var` 只提升声明，不提升赋值 → 声明前访问是 `undefined`
- `function` 声明整体提升（包括函数体）→ 可以在声明前调用
- 函数提升优先级 > 变量提升（同名时函数覆盖变量）
- 重复 `var` / `function` 声明不报错，后面的覆盖前面的
- 箭头函数、函数表达式不提升
- 提升只在当前作用域内生效（函数内的 `var` 提升到函数顶部，不是全局）

典型例子（这个两次打印 foo 要好好看）
