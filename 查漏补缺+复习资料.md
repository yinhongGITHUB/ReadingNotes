...
JavaScript 对象有两种属性类型：

1. **数据属性（Data Property）**：

   - 直接存储值的普通属性
   - 特性：`value`、`writable`、`enumerable`、`configurable`
   - 示例：`_val`、`name`、`age` 等

2. **访问器属性（Accessor Property）**：
   - 通过 getter/setter 方法访问的属性
   - 特性：`get`、`set`、`enumerable`、`configurable`
   - 示例：`val`（上面例子中）
...
// obj 的所有属性：
// - _val：数据属性（存储实际值）
// - val：访问器属性（读写时触发 get/set）

console.log(Object.keys(obj)); // ['_val', 'val']
console.log(Object.getOwnPropertyNames(obj)); // ['_val', 'val']

// 查看属性描述符
console.log(Object.getOwnPropertyDescriptor(obj, "_val"));
// { value: 0, writable: true, enumerable: true, configurable: true }

console.log(Object.getOwnPropertyDescriptor(obj, "val"));
// { get: [Function: get val], set: [Function: set val], enumerable: true, configurable: true }
```
...
console.log(person.fullName); // 'John Doe' (触发 getter)
person.fullName = "Jane Smith"; // 触发 setter
