...
1. 项目初始化：读取并解析 vite.confing.js 配置文件
   启动开发服务：基于 express 启动 http 服务器
   ESM 在古城公园：利用浏览器的原生 ESM 进行模块加载
   按需编译：试试编译请求的模块
   热模块替换（HMR）：通过 WebSocket 实现模块的局部更新
   Source Maps：自动生成 Source Maps，便于调试（Source Maps 是一种将编译后的代码（如压缩后的 JavaScript、转译后的 TypeScript/ESNext、CSS 预处理器生成的 CSS）映射回原始源代码的技术。）

##### 线上构建

项目初始化：读取并解析 vite.confing.js 配置文件
入口解析：设计图 Rollup 构建模块依赖图
