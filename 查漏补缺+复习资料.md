...
MessageChannel
script（整体代码执行）
鼠标移入（mouseenter/mouseover）、移出（mouseleave/mouseout）

##### 常见的微任务（MicroTask）有：

Promise.then / Promise.catch / Promise.finally 的回调
MutationObserver 的回调 监听 DOM 变化的 API，可以监控元素的属性、子节点、文本内容等的变化，并在变化时异步回调。
queueMicrotask
...
5. 请求方法非简单方法：
   非简单方法包括 PUT、DELETE、CONNECT、OPTIONS、TRACE、PATCH 等（简单方法仅 GET、POST、HEAD）。触发预检。
