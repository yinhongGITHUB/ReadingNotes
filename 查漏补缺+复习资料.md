...
- 定位：Node 早期同步模块规范，`require` + `module.exports`/`exports`，默认适配 `type:"commonjs"` 或 `.cjs`。
- 何时执行：代码在遇到 `require()` 时才同步加载并执行一次，产物缓存在 `require.cache`；重复 `require` 直接复用缓存。
- 循环依赖：执行到一半就把当前 `module.exports` 暴露，可能拿到半成品；后续重新赋值不会影响已拿到的引用。
- 绑定：导出是值拷贝（对象为同一引用）；`module.exports = 新值` 不会更新之前持有的旧引用。
- 作用域：文件被 IIFE 包裹，顶层 `this === module.exports`，可直接用 `__filename`/`__dirname`。

#### ESM 的特点和初次执行时机

- 定位：ECMAScript 标准模块，`import`/`export`，浏览器与 Node 原生支持（`type:"module"` 或 `.mjs`）。
- 编译期 vs 运行期：编译期只做静态解析依赖和绑定关系；真正执行用户代码发生在运行期，按依赖拓扑依次 evaluate。
- 何时执行：先解析 → 实例化 live binding → 执行；顶层 await 使模块变为异步模块，依赖它的模块会等待。
- 缓存：同一模块只实例化/执行一次，后续 import 共享同一实例和 live binding。
- 循环依赖：可用，最终能拿到正确值；但在赋值前访问会触发 TDZ 的 ReferenceError。
- 作用域：默认严格模式，顶层 `this === undefined`，无 `__filename/__dirname`，用 `import.meta.url`。
