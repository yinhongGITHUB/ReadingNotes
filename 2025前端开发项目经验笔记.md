...
帮我在项目根目录下，基于 openspec 工作流，自动生成 .claude/commands/opsx/ 目录，并为 apply、archive、bulk-archive、continue、explore、ff、new、onboard、sync、verify 这些命令生成标准的命令说明文档（md 文件），内容请参考 openspec 官方最佳实践。

每个 md 文件请包含：简介、输入参数、执行步骤、注意事项、输出等标准结构。
目录和文件内容要让 openspec/Claude Code Agent 能自动识别和驱动工作流。
