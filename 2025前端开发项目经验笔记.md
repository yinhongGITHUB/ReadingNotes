...
// 方式四：axios请求只需在请求头里设置 responseType 为 text 就可以获取到纯文本内容了
axios.get("/README.md", { responseType: "text" }).then((res) => {
  const mdContent = res.data; // 这里就是纯文本内容
...
```js
async function runWithConcurrency(tasks, max = 2) {
  let index = 0; // 当前准备发起请求的任务下标
  let active = 0; // 当前正在进行中的请求数量
  return new Promise((resolve, reject) => {
    let finished = 0;
    const total = tasks.length;
    function next() {
      if (finished === total) return resolve();
      while (active < max && index < total) {
        active++;
        const cur = index++;
        tasks[cur]()
          .then(() => {
          ...
          // .catch(reject); // 如果需要在任意请求失败时立即 reject 整体，可以启用这一行
          .catch(() => {
            active--;
            finished++;
            next();
...
const tasks = Array.from(
  { length: 100 },
  (_, i) => () => fetch(`https://api.example.com/data?id=${i}`)
);
