...
1. URL 解析：
   ...
   **在浏览器和操作系统中，都会维护一个 DNS 缓存表，用于记录域名与 IP 地址的对应关系，加快下次访问同一域名时的解析速度，减少 DNS 查询次数。**
2. DNS 查询：
   如果是 URL，浏览器会检查域名系统（DNS）以获取与该域名相关的 IP 地址。这一步可能涉及到缓存查找、递归 DNS 查询等。
   顺序是：本地 DNS 服务器/递归 DNS 服务器 => 根域名服务器 => 顶级域名服务器 => 权威域名服务器
3. 建立 TCP 连接：
